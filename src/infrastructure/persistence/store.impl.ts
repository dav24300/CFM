...
let postgresHydratedStore: Store | null = null;
let postgresBootstrapStarted = false;

function schedulePostgresHydration(): void {
  if (!process.env.DATABASE_URL || postgresHydratedStore || postgresBootstrapStarted) {
    return;
  }
  postgresBootstrapStarted = true;
  void import("@/infrastructure/persistence/db-adapter")
    .then(({ loadStoreFromPostgres }) => loadStoreFromPostgres())
    .then((store) => {
      if (store) {
        hydrateStoreFromPostgres(store);
        console.log("[CFM] Store hydraté depuis PostgreSQL");
      }
    })
    .catch((err) => {
      console.error("[CFM] PostgreSQL hydration error:", err);
      postgresBootstrapStarted = false;
    });
}

/** Hydratation synchrone au boot serveur (appelée depuis health check ou bootstrap). */
export async function bootstrapPostgresStore(): Promise<boolean> {
  if (!process.env.DATABASE_URL) return false;
  if (postgresHydratedStore) return true;
  const { loadStoreFromPostgres } = await import("@/infrastructure/persistence/db-adapter");
...
export function hydrateStoreFromPostgres(store: Store): void {
  postgresHydratedStore = store;
  fileStoreCache = store;
}

function loadSeedStore(): Store {
...
function migrateV2(store: Store): boolean {
  let changed = false;
...
function migrateV3(store: Store): boolean {
  let changed = false;
...
function ensureStore(): Store {
  ...
  schedulePostgresHydration();

  if (isServerlessRuntime()) {
    if (!memoryStore) {
      memoryStore = loadSeedStore();
    }
    return memoryStore;
  }

  if (process.env.DATABASE_URL && postgresHydratedStore) {
    return postgresHydratedStore;
  }
...
function saveStore(store: Store): void {
  ...
  if (process.env.DATABASE_URL) {
    postgresHydratedStore = store;
    void import("@/infrastructure/persistence/db-adapter")
      .then(({ saveStoreToPostgres }) => saveStoreToPostgres(store))
      .catch((err) => console.error("[CFM] PostgreSQL save error:", err));
  }
}
