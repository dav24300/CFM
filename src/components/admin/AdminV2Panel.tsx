    ...
    <div className="mt-8 space-y-10">
      <section>
        ...
        <div className="mt-4 space-y-2">
          {users.length === 0 ? (
            <p className="text-sm text-admin-muted">Aucun compte</p>
          ) : (
            users.map((u) => (
              <div key={u.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-admin-surface p-3 shadow text-sm">
                <div>
                  <strong>{u.first_name} {u.last_name}</strong> — {u.email}
                  <span className="ml-2 text-admin-muted">{u.membership_type} / {u.role}</span>
                </div>
      ...
      <section>
        ...
        <div className="mt-4 space-y-2">
          {familyLinks.map((l) => (
            <div key={l.id} className="flex flex-wrap justify-between gap-2 rounded-lg bg-admin-surface p-3 shadow text-sm">
              <span>
      ...
      <section>
        ...
        <div className="mt-4 overflow-x-auto rounded-xl bg-admin-surface shadow">
      ...
      {isAdmin && (
        <section>
          ...
          {showPetitionForm && (
            <form onSubmit={createPetition} className="mt-4 space-y-3 rounded-xl bg-admin-surface p-4 shadow">
          ...
          <div className="mt-4 space-y-2">
            {petitions.map((p) => (
              <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-admin-surface p-3 shadow text-sm">
                <div>
