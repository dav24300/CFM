...
export function PushNotificationForm({ onSend }: Props) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="font-display text-admin-h3 font-semibold text-admin-ink">Notification push</h2>
      <div className="flex flex-wrap gap-2">
        {PUSH_TEMPLATES.map((tpl) => (
          <button
            key={tpl.label}
            type="button"
            className="rounded-admin-ctrl border border-admin-border px-3 py-1.5 text-xs font-medium text-admin-muted transition-colors hover:bg-admin-bg hover:text-admin-ink"
      ...
      <Card className="max-w-lg p-4">
        <form onSubmit={submit} className="space-y-2">
          <NativeSelect
            ...
            onChange={(e) => setPushForm({ ...pushForm, topic: e.target.value })}
            className="text-sm"
          >
