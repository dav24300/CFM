...
describe("content.repository", () => {
  ...
  it("returns published news sorted by newest date", async () => {
    const rows = await getPublishedNews();
    expect(rows.map((n) => n.id)).toEqual([3, 1]);
  });

  it("adds newsletter email once and rejects duplicates", async () => {
    await addNewsletter("john@example.org");
    ...
    await expect(addNewsletter("JOHN@example.org")).rejects.toThrow("ALREADY_EXISTS");
    expect(mockStore.newsletter).toHaveLength(1);
  });

  it("computes admin stats with pending counters", async () => {
  ...
  it("adds membership/help/contact records", async () => {
    await addMembership({
      type: "famille",
      first_name: "Jean",
      last_name: "K",
    ...
    await addHelpRequest({ first_name: "M", message: "Need support" });
    await addContactMessage({ name: "A", email: "a@b.cd", message: "Hello" });
  ...
  it("covers admin create/update/delete workflows", async () => {
    await adminCreate("news", { title: "N1", content: "Body" });
    await adminCreate("studies", { title: "S1", content: "Study body" });
    await adminCreate("campaigns", { title: "C1", description: "desc" });
    await adminCreate("actions", { province: "Kin", title: "Action" });
    await adminCreate("testimonials", { content: "Temoignage", anonymous: "1" });
    await adminCreate("press_releases", { title: "Press", content: "CP" });
    ...
    await adminUpdateStatus("memberships", 10, "approved");
    await adminUpdateStatus("help_requests", 11, "processing");
    ...
    await adminDelete("news", firstNewsId);
    expect(mockStore.news.some((n: any) => n.id === firstNewsId)).toBe(false);
  });

  it("returns admin data bundles reversed collections", async () => {
